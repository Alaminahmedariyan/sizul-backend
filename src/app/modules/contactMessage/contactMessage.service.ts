...
import type { ContactMessageStatus } from "./contactMessage.interface";

const delegate =
  prisma.contactMessage as unknown as PrismaDelegate<ContactMessage>;
...
const createContactMessageInDB = async (payload: {
  name: string;
  email: string;
  phone?: string;
  company?: string;
  ...
  const contactMessage = await prisma.contactMessage.create({
    data: payload as Prisma.ContactMessageUncheckedCreateInput,
  });
...
export const getAllContactMessagesFromDB = async (
  query: Record<string, unknown>,
) => {
  const queryBuilder = new QueryBuilder<ContactMessage>(
    delegate,
...
const updateContactMessageStatusInDB = async (
  id: string,
  ...
  const repliedAt =
    status === "REPLIED" && !existing.repliedAt
      ? new Date()
      : existing.repliedAt;
