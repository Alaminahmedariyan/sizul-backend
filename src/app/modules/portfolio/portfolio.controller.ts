...
const getAllPortfoliosPublic = catchAsync(
...
const getAllPortfoliosAdmin = catchAsync(
  async (req: Request, res: Response) => {
    const { data, meta } = await portfolioService.getAllPortfoliosFromDB(
      req.query as Record<string, unknown>,
      { publicOnly: false },
...
const getPortfolioBySlugPublic = catchAsync(
  async (req: Request, res: Response) => {
    const portfolio = await portfolioService.getPortfolioBySlugFromDB(
...
const getPortfolioByIdAdmin = catchAsync(
  async (req: Request, res: Response) => {
    const portfolio = await portfolioService.getPortfolioByIdFromDB(
...
const updatePortfolio = catchAsync(async (req: Request, res: Response) => {
  const portfolio = await portfolioService.updatePortfolioInDB(
    req.params.id as string,
    req.body,
  );
  sendResponse(res, {
...
const updatePortfolioStatus = catchAsync(
  async (req: Request, res: Response) => {
    const portfolio = await portfolioService.updatePortfolioStatusInDB(
      req.params.id as string,
      req.body.status,
    );
    sendResponse(res, {
...
const addPortfolioImage = catchAsync(async (req: Request, res: Response) => {
  const image = await portfolioService.addPortfolioImageInDB(
    req.params.id as string,
    req.body,
  );
  sendResponse(res, {
...
const removePortfolioImage = catchAsync(async (req: Request, res: Response) => {
  await portfolioService.removePortfolioImageFromDB(
...
const linkPortfolioService = catchAsync(async (req: Request, res: Response) => {
  const link = await portfolioService.linkPortfolioServiceInDB(
    req.params.id as string,
    req.body.serviceId,
  );
  sendResponse(res, {
...
const unlinkPortfolioService = catchAsync(
  async (req: Request, res: Response) => {
    await portfolioService.unlinkPortfolioServiceFromDB(
      req.params.id as string,
